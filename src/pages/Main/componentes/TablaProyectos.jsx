...
import { actualizarProyEnBD } from '../helpers/StartEditProyect';
import { delProEnBD } from '../helpers/DelProyect';
import { ArchivarEnBD } from '../helpers/Archivar';

export const TablaProyectos = ({ cargarProyectos, setNuevoProyect, navigate, theme }) => {
  ...
  const proyectosFiltrados = cargarProyectos.filter(proyecto => proyecto.status === false);
  ...
  const [textoCampo, setTextoCampo] = useState('');
    ...
    const handleChange = (e) => {
        ...
        setProyectoEditar({ ...proyectoEditar, [name]: value });
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await actualizarProyEnBD(
                proyectoEditar.proyectId,
                proyectoEditar.companyName,
                proyectoEditar.address,
                proyectoEditar.task,
                proyectoEditar.taskDescription,
                proyectoEditar.formDate,
                proyectoEditar.untilDate,
                proyectoEditar.madeDate,
                proyectoEditar.resposiblePerson,
                proyectoEditar.weekDuration,
                proyectoEditar.hourDuration,
                proyectoEditar.costs,
                proyectoEditar.costsGoogle,
                proyectoEditar.clientId,
                
            );
            swal.fire("Erfolg", "Das Projekt wurde erfolgreich aktualisiert", "success");
        } catch (error) {
            console.error("Error al actualizar los datos del proyecto:", error);
            swal.fire("FEHLER", "Beim Aktualisieren des Projekts ist ein Fehler aufgetreten", "error");
        }
    };
                            ...
                            <tr>
                                ...
                                <td colSpan="10">
                                    <Accordion
                                     ...
                                     className="w-100" 
                                    activeKey={activeAccordion} onSelect={handleAccordionToggle}>
